...
export default function Attendance() {
    ...
    useEffect(() => {
        ...
        const loadAttendance = async () => {
            setLoading(true);
            try {
                // Load logged in user employee profile first to get their Mongoose _id
                ...
                // Filter records matching employee Mongoose _id
                const userRecords = list.filter((r) => {
                    let recEmpId = r.employeeId?._id || r.employeeId || "";
                    if (typeof r.employeeId === "object" && r.employeeId !== null) {
                        recEmpId = r.employeeId._id || r.employeeId.employeeId || "";
                    }
                    // Fallback to match by employee Name / Code just in case
                    const isMatch =
                        recEmpId === employeeDbId ||
                        r.employeeCode === loggedInUser?.employeeId;
                    return isMatch;
                });
                        ...
                        <table className="employee-table" style={{ width: "100%", borderCollapse: "collapse" }}>
                            <thead>
                                <tr style={{ borderBottom: "1px solid #e2e8f0" }}>
                                    ...
                                    <th style={{ padding: "14px 24px", textAlign: "left", fontSize: "11px", fontWeight: "700", color: "#64748b", textTransform: "uppercase" }}>Check In</th>
                                    <th style={{ padding: "14px 24px", textAlign: "left", fontSize: "11px", fontWeight: "700", color: "#64748b", textTransform: "uppercase" }}>Check Out</th>
