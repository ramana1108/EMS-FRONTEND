...
export default function Employee() {
  ...
  const coworkerDepartmentId = useMemo(() => {
    ...
    return currentEmployeeProfile.departmentId || currentEmployeeProfile.departmentName || null;
  }, [currentEmployeeProfile]);

  const coworkerDepartmentName = useMemo(() => {
    if (!currentEmployeeProfile) return null;
    return currentEmployeeProfile.departmentId?.departmentName || currentEmployeeProfile.departmentName || null;
  }, [currentEmployeeProfile]);
  ...
  const coworkers = useMemo(() => {
    ...
    return employees.filter((emp) => {
      const empDeptId = emp.departmentId && typeof emp.departmentId === "object" ? emp.departmentId._id : emp.departmentId;
      const empDeptName =
        emp.department || (emp.departmentId && typeof emp.departmentId === "object" ? emp.departmentId.departmentName : undefined);
  ...
  const handleOpenEnrollModal = () => {
    ...
    setFormData({
      ...
      departmentId: departmentsOptions[0]?._id || "",
  ...
  const handleFormSubmit = async (e) => {
    ...
    const newEmployeeRecord = {
      ...
      department: formData.department || formData.departmentId,
  ...
  const departmentsList = Array.from(new Set(displayedEmployees.map((e) => e.department || e.departmentId).filter(Boolean)));
  ...
  const inactiveStaffCount = displayedEmployees.filter(
    (e) => e.status?.toLowerCase() === "inactive" || e.status?.toLowerCase() === "on leave" || e.status?.toLowerCase() === "offboarded"
  ).length;
  ...
  return (
    <div>
      ...
      <div className="page-header">
        ...
        <button className="btn-add-dept" onClick={handleOpenEnrollModal} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          <Plus size={16} />
        ...
        <div className="table-responsive">
          <table className="employee-table table-fixed">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan="5" style={{ textAlign: "center", padding: "40px 0" }}>Loading employee records...</td>
                </tr>
              ) : filteredEmployees.length === 0 ? (
                <tr>
                  <td colSpan="5" style={{ textAlign: "center", padding: "40px 0" }}>No coworkers match filter criteria.</td>
                </tr>
              ) : (
                  ...
                  <tr key={index} className="employee-row">
                    <td>
                      <div className="employee-avatar-wrapper">
                        <div className="employee-avatar-circle">{getInitials(emp.name)}</div>
                        <div>
                    ...
                    <td>
                      <span className="employee-dept-pill">{emp.department || emp.departmentId || ""}</span>
                    </td>

                    <td>
                      <div>
                        <p className="employee-role-type">{emp.employmentType || emp.role || ""}</p>
                        <p className="employee-joining-date">{emp.joiningDate ? `Joining: ${emp.joiningDate}` : ""}</p>
                      </div>
                    </td>
                    ...
                    <td className="table-actions-col">
                      <div className="employee-action-buttons">
                        ...
                        <button
                          ...
                          onClick={() => handleDeleteClick(emp._id || emp.employeeId)}
                        >
                          <Trash2 size={16} />
                        </button>
                      </div>
          ...
          <div className="modal-content-card-wide">
            ...
            <form onSubmit={handleFormSubmit} className="enroll-form">
              ...
              <div className="form-grid-2">
                ...
                <div className="form-group">
                  <label>
                    Created By <span className="req">*</span>
                  </label>
                  <input
                    type="text"
                    name="createdBy"
              ...
              <div className="form-grid-2">
                ...
                <div className="form-group">
                  <label>
                    Salary ($ / year) <span className="req">*</span>
                  </label>
                  <input
                    type="number"
                    name="salary"
          ...
          <div className="modal-content-card view-card">
            ...
            <div className="profile-details-grid">
              <div className="detail-item">
                <strong>ID:</strong> {viewingEmployee.employeeId}
              </div>
              <div className="detail-item">
                <strong>Name:</strong> {viewingEmployee.name}
              </div>
              <div className="detail-item">
                <strong>Email:</strong> {viewingEmployee.email}
              </div>
              <div className="detail-item">
                <strong>Phone:</strong> {viewingEmployee.phone || "N/A"}
              </div>
              <div className="detail-item">
                <strong>Gender:</strong> {viewingEmployee.gender || "N/A"}
              </div>
              <div className="detail-item">
                <strong>Date of Birth:</strong> {viewingEmployee.dob || "N/A"}
              </div>
              <div className="detail-item">
                <strong>Department:</strong> {viewingEmployee.department || viewingEmployee.departmentId}
              </div>
              <div className="detail-item">
                <strong>Employment Type:</strong> {viewingEmployee.employmentType || viewingEmployee.role}
              </div>
              <div className="detail-item">
                <strong>Joining Date:</strong> {viewingEmployee.joiningDate}
              </div>
              <div className="detail-item">
                <strong>Salary:</strong> ${viewingEmployee.salary || "N/A"}
              </div>
              <div className="detail-item">
                <strong>Status:</strong> {viewingEmployee.status}
              </div>
              <div className="detail-item">
                <strong>Created By:</strong> {viewingEmployee.createdBy?.name || viewingEmployee.createdBy || ""}
              </div>
